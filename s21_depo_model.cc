...
std::queue<std::pair<std::tm, double>> DepoModel::EvaluateDepoData() {
  ...
  while (counter < d_end_ && !zero_balance_) {
    ...
    // checks multimaps and if date finds itself in multimap the "answer_" adds
    // or looses money
    AddDelMoney(counter, &buff_sum);
    // counts current interest by adding daily interest in buffer and changing
    // date by 1 in a loop. For every day checks with "d_chekpoints_" of
    // payments. zero balance is inserted to check if withdrawals after
    // "AddDelMoney(..)" were bigger than the sum on depo.
    if ((counter == d_checkpoints_.front() && !zero_balance_) ||
      ...
      factor = 0;
      d_checkpoints_.pop();
      answ_.push(std::pair<std::tm, double>(*(std::localtime(&counter)), plus));
