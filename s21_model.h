...
// MODEL is the engine for computation. Reverse Polish Notation (hereinafter RPN) is used.
// Dijkstra's algorithm is used for parsing of expression. NODE is a specific chunck for
// storing of operators and operands (hereinafter ELEM) with storage of relevant properties.
...
// l_br_num and r_br_num are counts of left and right brackets. Can be incremented by func.
    void ParseBrackets(size_t &i, size_t &l_br_num, size_t &r_br_num);
    void ParseBinary(size_t &i);
    void ParseOperands(size_t &i);

// Checks common omission of '*' in math expressions, which are clear for human
// but does not understandable for machine. Supplements expression with '*' where needed.
...
// Checks corectness of the expression regarding to binary operands usage.
    ...
    long double ComputeBinary(long double num_1, long double num_2,
                               Action act);
};
