...
class MainWindow : public QMainWindow {
   ...
   private slots:
// just writes text from button to the end of the string to be passed to model.
// used by the majority of presented buttons.
    void on_add_clicked();
// clears all data and sets settings to defaults.
    void on_AC_clicked();
// passes the string to model via controller. model computes given expression and
// passes result to model. result is shown to user.
    void on_result_clicked();
// reads and validates x and y border values when cursor activated certain fields
// and changed values in them.
...
// clears themseve, creates new table, pass input info to credit_model via controller.
// gets the answer. uses it for filling table. output the result.
...
// sets values to defaults and recreates table with 0 rows.
 ...
 // passes data through controller. recieves results. clears table and set default
 // fills in the table. outputs it.
