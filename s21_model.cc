...
void Model::ParseString() {
    size_t i = 0;
    size_t l_br_num = 0;  // stores the number of left brackets for check of correct brackets usage
    size_t r_br_num = 0;  // stores the number of right brackets for check of correct brackets usage
    ...
    if (check != Action::kVariable && check != Action::kNumber && check != Action::kRightBracket) {
        throw -12;
    }
}

// duplication of "CheckUnForMulitplicatorAbsence()" is because ONE function call can
// find and create NODES of several consecutive unary operands. Can be rewritten into more compact
// and readable form (using "ELIF") but would create only 1 operand during one call.
void Model::ParseUnary(size_t &i) {
    if (expr_[i] == 'c' && expr_[i + 1] == 'o' && expr_[i + 2] == 's') {
        Model::CheckUnForMulitplicatorAbsence();
        parsed_.std::queue<Node>::push(Node(Action::kCos, Priority::kHight, OperatorType::kUnary));
        i += 3;
    }
    if (expr_[i] == 's' && expr_[i + 1] == 'i' && expr_[i + 2] == 'n') {
        Model::CheckUnForMulitplicatorAbsence();
        parsed_.std::queue<Node>::push(Node(Action::kSin, Priority::kHight, OperatorType::kUnary));
        i += 3;
    }
    if (expr_[i] == 't' && expr_[i + 1] == 'a' && expr_[i + 2] == 'n') {
        Model::CheckUnForMulitplicatorAbsence();
        parsed_.std::queue<Node>::push(Node(Action::kTan, Priority::kHight, OperatorType::kUnary));
        i += 3;
    }
    if (expr_[i] == 'a' && expr_[i + 1] == 'c' && expr_[i + 2] == 'o' &&
        expr_[i + 3] == 's') {
        Model::CheckUnForMulitplicatorAbsence();
        parsed_.std::queue<Node>::push(Node(Action::kAcos, Priority::kHight, OperatorType::kUnary));
        i += 4;
    }
    if (expr_[i] == 'a' && expr_[i + 1] == 's' && expr_[i + 2] == 'i' &&
        expr_[i + 3] == 'n') {
        Model::CheckUnForMulitplicatorAbsence();
        parsed_.std::queue<Node>::push(Node(Action::kAsin, Priority::kHight, OperatorType::kUnary));
        i += 4;
    }
    if (expr_[i] == 'a' && expr_[i + 1] == 't' && expr_[i + 2] == 'a' &&
        expr_[i + 3] == 'n') {
        Model::CheckUnForMulitplicatorAbsence();
        parsed_.std::queue<Node>::push(Node(Action::kAtan, Priority::kHight, OperatorType::kUnary));
        i += 4;
    }
    if (expr_[i] == 's' && expr_[i + 1] == 'q' && expr_[i + 2] == 'r' &&
        expr_[i + 3] == 't') {
        Model::CheckUnForMulitplicatorAbsence();
        parsed_.std::queue<Node>::push(Node(Action::kSqrt, Priority::kHight, OperatorType::kUnary));
        i += 4;
    }
    if (expr_[i] == 'l' && expr_[i + 1] == 'n') {
        Model::CheckUnForMulitplicatorAbsence();
        parsed_.std::queue<Node>::push(Node(Action::kLn, Priority::kHight, OperatorType::kUnary));
        i += 2;
    }
    if (expr_[i] == 'l' && expr_[i + 1] == 'o' && expr_[i + 2] == 'g') {
        Model::CheckUnForMulitplicatorAbsence();
        parsed_.std::queue<Node>::push(Node(Action::kLog, Priority::kHight, OperatorType::kUnary));
        i += 3;
    }
}
...
void Model::ParseOperands(size_t &i) {
    ...
    if (expr_[i] == 'x') {
        CheckUnForMulitplicatorAbsence();
        parsed_.std::queue<Node>::push(Node(Action::kVariable, Priority::kVal, OperatorType::kOperand));
        ++i;
    }
    if (expr_[i] == 'e') {
        CheckUnForMulitplicatorAbsence();
...
// contains some clearly visible conditions to define "+" or "-" as unary operands.
void Model::ParseBinary(size_t &i) {
    if (expr_[i] == '+') {
        if (!parsed_.std::queue<Node>::empty()) {
            Action check = parsed_.std::queue<Node>::back().act_;
            if (check != Action::kNumber && check != Action::kRightBracket && check != Action::kVariable) {
                ++i;
            } else {
                parsed_.std::queue<Node>::push(Node(Action::kAdd, Priority::kLow, OperatorType::kBin));
            }
        }
    }
    if (expr_[i] == '-') {
        if (parsed_.std::queue<Node>::empty()) {
            parsed_.std::queue<Node>::push(Node(Action::kUnaryMinus, Priority::kHight, OperatorType::kUnary));
        } else {
            Action check = parsed_.std::queue<Node>::back().act_;
            if (check != Action::kNumber && check != Action::kRightBracket && check != Action::kVariable) {
                parsed_.std::queue<Node>::push(Node(Action::kUnaryMinus, Priority::kHight, OperatorType::kUnary));
            } else {
                parsed_.std::queue<Node>::push(Node(Action::kSub, Priority::kLow, OperatorType::kBin));
            }
        }
        ++i;
    }
    if (expr_[i] == '*') {
        CheckBinaryForErr(i);
        parsed_.std::queue<Node>::push(Node(Action::kMul, Priority::kMedium, OperatorType::kBin));
        ++i;
    }
    if (expr_[i] == '/') {
        CheckBinaryForErr(i);
        parsed_.std::queue<Node>::push(Node(Action::kDiv, Priority::kMedium, OperatorType::kBin));
        ++i;
    }
    if (expr_[i] == '^') {
        CheckBinaryForErr(i);
        parsed_.std::queue<Node>::push(Node(Action::kPow, Priority::kHight, OperatorType::kBin));
        ++i;
    }
    if (expr_[i] == 'm' && expr_[i + 1] == 'o' && expr_[i + 2] == 'd') {
        CheckBinaryForErr(i);
        parsed_.std::queue<Node>::push(Node(Action::kMod, Priority::kMedium, OperatorType::kBin));
        i += 3;
    }
}
...
void Model::ParseBrackets(size_t &i, size_t &l_br_num, size_t &r_br_num) {
    if (expr_[i] == ')') {
        ...
        Action check = parsed_.std::queue<Node>::back().act_;
        if (check != Action::kNumber && check != Action::kVariable && check != Action::kRightBracket) {
            throw static_cast<int>(i);
        }
        parsed_.std::queue<Node>::push(Node(Action::kRightBracket, Priority::kLow, OperatorType::kBracket));
        r_br_num += 1;
        ++i;
    }
    if (expr_[i] == '(') {
        CheckUnForMulitplicatorAbsence();
        parsed_.std::queue<Node>::push(Node(Action::kLeftBracket, Priority::kLow, OperatorType::kBracket));
        ++i;
        l_br_num += 1;
    }
}
    ...
    while (i > 0) {
        ...
        if (buff.act_ == Action::kNumber || buff.act_ == Action::kVariable) {
            ...
            while (operators_.std::stack<Node>::top().act_ != Action::kLeftBracket) {
                polish_.std::queue<Node>::push(
                    operators_.std::stack<Node>::top());
            ...
            if (!operators_.empty() &&
// the sec condition is for "right-to-left" computation of sequentional powers
// (2^3^3 = 2^[3^3])
                !(buff.act_ == Action::kPow && operators_.top().act_ == Action::kPow)) {
                while (!operators_.empty() &&
                       operators_.top().prior_ >= buff.prior_ &&
