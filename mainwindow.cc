...
void MainWindow::on_result_clicked() {
    ...
    if (return_code == -1) {
        ...
        if (x_validator_.validate(buff, not_used) == QValidator::Acceptable) {
            ...
            ui->out_msg->setText(
                QString::fromStdString(err_handler.TransmitMsg(-6)));
            return;
        }
...
void MainWindow::on_x_y_editingFinished() {
    ...
    if (x_validator_.validate(buff, not_used) == QValidator::Acceptable) {
        ...
        ui->out_msg->setText(
            QString::fromStdString(err_handler.TransmitMsg(-5)));
    }
}
...
void MainWindow::on_draw_clicked() {
    if (x_left_ >= x_right_ || y_top_ <= y_bot_) {
        ui->out_msg->setText(
            QString::fromStdString(err_handler.TransmitMsg(-4)));
    } else {
        ...
        if (return_code == -1) {
            ...
            ui->out_msg->setText(QString("Mistakes! (The first is at pos %1)")
                                     .arg(return_code + 1));
        } else {
            ui->out_msg->setText(
                QString::fromStdString(err_handler.TransmitMsg(return_code)));
...
void MainWindow::on_credit_evaluate_clicked() {
    ...
    if (c_sum_ != 0 && c_interest_ != 0 && c_period_ != 0) {
        ...
        ui->credit_table->setHorizontalHeaderLabels(QStringList()
                                                    << "Month"
                                                    << "Sum"
                                                    << "Interest"
                                                    << "Main debt");
        ...
        for (int i = 0; i < ui->credit_table->rowCount(); ++i) {
            for (int j = 0; j < ui->credit_table->columnCount(); ++j) {
                ...
                switch (j) {
                    case 0:
                        cell->setText(
                            QString("Month %1").arg(QString::number(i + 1)));
                        break;
                    case 2:
                        cell->setText(
                            QString::number(percents.front(), 'f', 2));
                        overpay += percents.front();
                        percents.pop();
                        break;
                    case 3:
                        cell->setText(
                            QString::number(main_debt.front(), 'f', 2));
                        main_debt.pop();
                        break;
                    case 1:
                        cell->setText(QString::number(
                            percents.front() + main_debt.front(), 'f', 2));
                        break;
                }
...
void MainWindow::on_d_eval_clicked() {
    ...
    if (depo_.d_start != 0 && depo_.d_sum != 0 && depo_.d_term != 0 &&
        ...
        ui->out_msg_3->setText(
            "Please check the corectness of the data written");
    }
}
